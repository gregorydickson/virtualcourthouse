...
if (user.hasRole('ROLE_SUPERVISOR')) {
	tabData << [name: 'assignIndexers',    icon: 'icon_user', messageCode: 'spring.security.ui.user.assignIndexers']
	isSupervisor = true;
}
if (user.hasRole('ROLE_INDEXER')) {
	tabData << [name: 'assignReviewer',    icon: 'icon_user', messageCode: 'spring.security.ui.user.assignReviewer']
	isIndexer = true;
}
...
<s2ui:tabs elementId='tabs' height='375' data="${tabData}">
	...
	<g:if test='${isSupervisor}'>
		<s2ui:tab name='assignIndexers' height='275'>
			<g:each var="entry" in="${allIndexers}">
			<div>
				<g:checkBox name="${entry.username}" value="${entry}"/>${entry.username.encodeAsHTML()}
			</div>
			</g:each>
		</s2ui:tab>
	</g:if>
	...
	<g:if test='${isIndexer}'>	
		<s2ui:tab name='assignReviewer' height='275'>
			<g:each var="entry" in="${roleMap}">
			<div>
				<g:checkBox name="${entry.key.authority}" value="${entry.value}"/>
				<g:link controller='role' action='edit' id='${entry.key.id}'>${entry.key.authority.encodeAsHTML()}</g:link>
